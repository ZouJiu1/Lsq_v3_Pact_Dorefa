...
from quantization.lsq_v3 import prepare as lsq_v3
...
def trainer():
    ...
    quantization_method = "dorefa"       #[dorefa, pact, lsq_v3]
    ...
    if Floatmodel:
        ...
        prefix = 'pact'
    elif quantization_method=='lsq_v3':
        prefix = 'lsq_v3'
    else:
    ...
    elif quantization_method=='lsq_v3' and not Floatmodel:
        #lsq_v3
        lsq_v3(model, inplace=True, a_bits=config["a_bit"], w_bits=config["w_bit"],
                all_positive=config["all_positive"], batch_init = config["batch_init"])
        print(model, '\npreparing lsq_v3 models')
    elif Floatmodel:
